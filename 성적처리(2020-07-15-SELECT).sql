...
-- Recored의 제한(전체 데이터수 > 조건에 맞는 데이터 개수)
-- WHERE 절에 조건을 설정하여 데이터를 조회
-- PK< UNIQUE로 설정된 칼럼을 단일 조건으로 SELECTION할때는
--      이 데이터는 1개의 객체로 취급한다. (VO에 담는다)
SELECT * FROM tbl_student
WHERE st_num = '20001';

-- st_num에 저장된 값이 '20001'이상 이고(AND) '20010'이하인 조건에 맞는
-- 데이터리스트(Record)만 보여달라
-- PK, UNIQUE로 설정된 칼럼을 복합 조건절로 SELECTION할떄는
...
-- SELECTION 을 수행했을떄 나타나는 레코드가 1개 일지라도
--      이 데이터는 리스토로 취급한다.
SELECT * FROM tbl_student
WHERE st_name = '독고예준';
...
-- 비교연산자(=)와 관계연산자 OR 를 조합하여 SELECTION 구현하기
SELECT st_num, st_name, st_dept, st_grade
FROM tbl_student
WHERE st_num = '20001' OR st_num ='20003' OR st_num = '20006';

-- 한개의 칼럼에 다수의 OR 조건을 부여하여
-- SELECTION을 수행할떄는 IN 연산자를 사용하여 구현할수 있다.
SELECT st_num, st_name, st_dept, st_grade
FROM tbl_student
