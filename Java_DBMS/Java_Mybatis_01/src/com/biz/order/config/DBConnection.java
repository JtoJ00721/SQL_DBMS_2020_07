...
public class DBConnection {
	...
	static {
		...
		try {
			...
			// JVM이 관리하는 객체가 된다.(static이기 때문에)
			// 만약 이 프로젝트를 어떤경로로 2번 이상 작동을 시킨다면
			// 작동되는 횟수만큼  sqlSessionFactory가 계속해서 생성될 것이다.
			// sqlSessionFactory 객체는 하는 일이 꽤 많아서 생성하는 것 자체만으로
			// JVM입장에서는 부담이 될수 있다.
			// sqlSessionFactory를 생성하기 전에
			// 누군가 만들어서 사용중이라면 다시 만들지 않고 재활용할 수 있도록
			// 해주는 것이 좋다.
			...
			System.out.println("SqlSwssionFactory 생성중 xml 파일 문제 발생");
			e.printStackTrace();
		}
