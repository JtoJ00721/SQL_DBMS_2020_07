...
public class OrderServiceImplV1 implements OrderService {
	...
	/*
	 * throws Exception try-catch를 사용하여 자체적으로 exception을 Handdling(처리)를 하였는데
	 * try-catch가 코드내에 많아지면 코드의 가독성이 매우 떨어진다.
	 * 
	 * exception 처리를 직접 하지 않고 이 메서드를 호출한 곳에 대리요청한다(exception을 던진다)
	 */

	/*
	 * insert를 호출하는 곳에서는 매개변수로 전달해야 하며 try-catch로 exception처리를 하고 결과값으로 정수값을 받아서
	 * insert유무를 처리해야 한다.
	 */
	@Override
	public int insert(OrderVO orderVO) throws SQLException {
		...
		pSt.setNString(1, orderVO.getO_num()); // 2
	...
	/*
	 * selectAll() method를 호출하는 곳에서는 우무런 매개변수 없이 method를 호출하고 exception 처리를 수행해야 하면
	...
	/*
	 * selectAl;l()과 findBySeq() method에서
	 * ResultSet으로 부터 데이터를 getter 하여
