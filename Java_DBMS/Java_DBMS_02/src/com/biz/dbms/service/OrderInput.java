...
public class OrderInput {
	...
	public boolean orderUpdate() {
		...
		System.out.print("수정할 SEQ번호를 입력(quit : 종료) >>");
		...
		if(orderVO == null) {
			System.out.println("SEQ에 해당 데이터가 없습니다 ><");
			return true;
		}
		// 여기까지 정상 수행되면
		// 입력한 SEQ에 해당하는 데이터가 orderVO에 담겨 있을것이다.
		...
		if(!str_num.isEmpty()) {
			...
			while(true) {
			System.out.printf("변경할 가격(%s,Enter:변경안함, quit:종료) >> "
					,orderVO.getO_price());
			String str_price = scan.nextLine();
			if(str_num.equals("quit")) {
				return false;
			...
			while(true) {
				System.out.printf("변경할 수량(%s,Enter:변경안함, quit:종료) >> "
						,orderVO.getO_qty());
				String str_qty = scan.nextLine();
				if(str_num.equals("quit")) {
					return false;
	...
	public OrderVO detailView() {
		...
		System.out.println("자세히 확인할 SEQ 입력 >> ");
		String strSeq = scan.nextLine();
	...
	public boolean orderDelete() {
		...
		try {
			...
			System.out.println("시퀀스는 숫자만 가능 ><");
			return true;
		}
