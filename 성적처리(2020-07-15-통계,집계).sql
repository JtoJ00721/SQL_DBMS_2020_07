...
-- tbl_student 칼럼중에 st_grade, st_age는 숫자형 칼럼인데
...
--      리스트로 보여준다면, 어떠한 학과가 있는지 알기 쉬울것 같다.
...
-- st_name 칼럼에 저장된 값들이 중복된 값들이 없어서 
-- GROUP BY 가 의미가 없는경우도 있다.
SELECT st_name FROM tbl_student
GROUP BY st_name;


--데이터를 학과이름으로 그룹을 짓고
-- 각 학과에 소속된 학생들이 몇명씩 인가를 알고싶다.

-- st_dept칼럼으로 GROUP을 만들고
-- 그 GROUP에 속하는 레코드가 몇개인가를 카운트하여 보여준다.
SELECT st_dept, COUNT(*) FROM tbl_student
GROUP BY st_dept;

-- 학년별로 소속된 학생이 몇명인가 알고싶다.
-- st_grade 칼럼으로 GROUP을 만들고
-- 각 그룹에 속하는 레코드가 몇개인가를 COUNT하여 보여준다.
...
SELECT st_grade, COUNT(st_grade), COUNT(st_name) FROM tbl_student
GROUP BY st_grade;

-- 각 학과별로 소속된 학생들의 총 나이 합계가 알고싶다.
