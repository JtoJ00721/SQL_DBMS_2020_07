...
-- 지금 1개의 값만 보일지라도 이 결과는 리스트(다수) 형으로 출력된다.
SELECT * FROM tbl_student
WHERE st_name = '남동예';

-- PK칼럼에 2개 이상의 조건을 부여하여 조회하기
-- 만약 결과가 1개의 Recode만 보일지라도 
-- 결과는 리스트(다수) 형으로 출력된다(취급한다)
SELECT * FROM tbl_student
WHERE st_num = '20010' OR st_num = '20020' OR st_num = '20030' ;

-- 1개의 칼럼에 다수의 OR 조건을 부여하여 SELECTION을 할떄는
-- IN 연산자를 사용할수 있다.
SELECT * FROM tbl_student
WHERE st_num IN ('20010','20020','20030');

-- 1개의 칼럼 값을 범위로 제한하는 SELECTION을 할떄
-- 부등호연산은 숫자 칼럼에만 적용되는 것이 원칙이나
-- 문자열일경우에도 저장된 데이터의 길이가 모두 같고
-- 패턴이 같으면 문자열도 부등호 연산이 된다.
SELECT * FROM tbl_student
WHERE st_num >= '20010' AND st_num <= '20030';

-- 범위를 지정할때 시작값과 종료값이 포함되는 연산을 수행할때는
...
-- 학생의 이름의 오름차순으로 전체리스트를 정렬하여 보여달라(ASC생략 가능)
SELECT * FROM tbl_student
...
SELECT * FROM tbl_student
WHERE st_num BETWEEN '20001' AND '20010'
ORDER BY st_dept, st_name DESC; -- dept뒤에 ASC생략함
...
-- 학번을 다음과 같이 구성
-- 20001 ~ 20100
-- 어떤 칼럼의 값을 정할때 규칙을 만들고 해당 칼럼을 PK로 삼아서
...
GROUP BY SUBSTR(sc_num,1,4)
ORDER BY 반; -- ORDER BY 는 SELECT의 명령문의 절들 중에 가장 끝에 나타나야 한다.
