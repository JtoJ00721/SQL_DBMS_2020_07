...
SELECT *
FROM tbl_score
WHERE sc_num BETWEEN '20001' AND '2001';

-- EQ(완전) JOIN
-- sc_score Table에 있는 학번의 정보는 반드시 st_student에 있다 라는 
--      전제하에 결과가 원하는 대로 나온다
--      FROM 다음에 JOIN할 테이블을 나열하고
--      WHERE절에 두 테이블의 연결점 칼럼을 설정해 주면 된다.
SELECT sc_num, st_name, sc_kor, sc_eng, sc_math, sc_music, sc_art
FROM tbl_score, tbl_student
WHERE
-- sc_num BETWEEN '20001' AND '2001' AND
sc_num = st_num;


-- OUTER JOIN
-- 성적 테이블에는 1~100까지 데이터가 있고
-- 학생 테이블에는 1~50까지만 데이터가 있다.
-- 이러한 상황에서 성적리스트를 확인하면서 학생정보를 연동하여 보고 싶을 때
-- EQ JOIN을 사용하게 되면 실제 데이터가 1~ 50까지만 나타나는 현상이 발생한다.
-- 이런상황에서 성적 Table의 데이터는 모두 확인하면서
--      학생정보테이블에 있는 정보만 연결해 보여주는 방식의 JOIN
...
-- JOIN이라는 키가 있으면 조건을 WHERE가 아닌 ON 뒤에 쓴다.
SELECT sc_num, st_name, sc_kor, sc_eng, sc_math, sc_music, sc_art
FROM tbl_score
...
-- 왼쪽테이블의 데이터가 잘 입력되었나 검증하는 용도로 많이 사용하고
-- 아직 FK설정이 되지 않은 table간에 정보를 리스트 업하는 용도로 사용된다.

SELECT sc_num, st_name, sc_kor, sc_eng, sc_math, sc_music, sc_art
FROM tbl_score --전부 가져오기
    LEFT JOIN tbl_student -- 연결하기
        ON sc_num = st_num;
