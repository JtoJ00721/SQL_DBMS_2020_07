...
-- 표준 SQL
-- CASE WHEN 조건문 THEN true일떄값 ELSE false일떄값 END
-- 오라클 전용
...
-- 만약 일련번호와 같은 칼럼을 사용하여 PK로 만들고 싶을 떄는 상당히 곤란한 상황이 된다.
...
-- 이 호출자를 호출할떄마다(SELECT, INSERT등에서) 내부에 가지고 있는 변수값을
-- 이 객체.NEXTVAL 이 가지고 있는 값을
-- INSERT를 수행할떄 PK인 seq칼럼에 주입을 하면 AUTO_INCREMENT효과를 낼수있다.
