...
--      특정한 칼럼을 하나 마련하고 그 칼럼에 데이터의 사용 유무를 등록하여
--      관리하는 것이 실무상 원칙이다.

-- 변경, 삭제 이상을 방지 : 특별한 경우가 아니면 2개 이상의 
--      Recode에 변화가 발생하지 않도록 코드나 관리를 설계해야한다.

-- 어떤 테이블에 변경될 가능성이 1 이라도 존재하는 칼럼이 있는데
--      여기에 동일한 값들이  중복되어서 저장되어 있을 때
--     이 칼럼의 값은 여러 레코드가 변경, 삭제되는 일이 생길수가 있다.
...
-- 삭생정보를 조회하면서 학과 테이블과 연결하여 조회

--기본 SELECT를 시작
-- FROM 절에 보고자하는 주 Table(MASTER)을 작성
...
-- JOIN 이 된후에 보조테이블의 칼럼을 WHERE 조건설정을 하여 SELECTION을 수행할수 있다.
SELECT st_num, st_name, st_dept, d_name, tbl_dept.d_prof, d_tel, st_grade, st_tel, st_addr
FROM tbl_student
    LEFT JOIN tbl_dept
        ON st_dept = d_code
...
-- 다른 테이블에 같은 칼럼 이름이 존재할 수 있다.
-- 보통은 칼럼에 prefix를 붙여 그 구분을 명확하게 하지만
-- table이 많아지다보면 같은 이름의 칼럼이 존재 할소 있다.
-- 이러한 상황에서 JOIN을 수행하면 칼럼을 제대로 인식하지 못하여
-- 오류가 발생하는 경우가 있다.(실제로 많다)
-- 이런 경우 table 에 Alias를 설정하여 주면 오류를 막을 수 있다.
...
-- VIEW 는 실제 Table과 똑같이 SELECT 명령을 통해 데이터를 조회할 수 있다.
