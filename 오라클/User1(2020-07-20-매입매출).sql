...
-- 00 회사의 매입매출 관리 프로잭트를 수행하고 있다.
-- 이 회사는 기존에 엑셀을 이용하여 거래처를 매입, 매출 관리를 수행해 왔는데
-- 최근 App을 개발하여 데이터베이스화 하고 관리를 수행하려고 한다.
-- 엑셀데이터를 받아서 DB로 보내기 위해 변환 작업을 수행하고 있다.

-- 엑셀 매입매출원장을 DB로 만들기 위해 매입, 매출을 분리하과, 날짜를 문자열화 했다.
-- 이 데이터를 DB에 저장하려고 봤더니, 이 데이터에 PK칼럼으로 지정할 만한 칼럼을 찾을 수 없다.
-- 이러한 경우 (Work DB인 경우)에는 데이터와 별도로 임의의 PK칼럼을 작성해서 관리해 주는 것이 좋다
...
io_deco	nVARCHAR2(125)	NOT NULL	,
...
--데이터를 확인하다 보니 단가와 금액이 0인 데이터가 있다.
SELECT * FROM tbl_iolist
WHERE io_price = 0 OR io_amt = 0;

-- 투니스, 7+8칫솔, 레종블루
-- 이 세가지 데이터가 거래된 5, 10, 28 seq 데이터가 칼럼값이 0이다.
-- 이 데이터의 단가, 금액을 수정하기 위해 동일한 상품 데이터가 있는지 확인작업
SELECT * FROM tbl_iolist
WHERE io_pname IN ('투니스', '7+8칫솔', '레종블루');
-- 검생르 해봤더니 위 3가지 상품의 거래 내역이 다른 것이 없다.
...
-- 반드시 PK칼럼값을 조회하고 PK 칼럼으로 WHERE절을 사용하자
...
-- 원본 데이터
--      수량, 매입단가, 매입금액, 매출금액 으로 칼럼을 분리하여 사용해 왔다.
--      이러한 형태의 데이터는 각 칼럼에 NULL값이 존재하여, 다양한 연산을 수행할때
--      문제를 일으킬 수 있다.
-- 변환된 데이터
--      수량, 단가, 금액 형식으로 칼럼을 통합작업을 수행했다.
--      칼럼으로 분리된 값들ㅇ르 공통 칼럼을 선정하고 데이터를 통합하는 것을 보편적으로 제 3 정규화가 되었다

-- 변환된 데이터를 참조하여
--      매입과 매출을 구분하여 보고 싶다 할떄
--      (거래처, 수량, 매입단가, 매입금액, 매출단가, 매출금액 형식으로 보고 싶을 떄)
--      이 데이터는 PIVOT 행태로 변환을 해야 한다.

SELECT * FROM tbl_iolist
-- WHERE (io_seq > 0 AND io_seq <= 10) OR (io_seq >= 300 AND io_seq <= 310)
ORDER BY io_dname ASC;

-- PIVOT 으로 데이터 펼쳐보기
--      매입 매출단가 가 단가(io_price) 칼럼에 같이 저장되어있다.
--      이 값은 io_inout 칼럼의 값을 모르면 매입인지 매출인지 구별이 어렵다.
--       따라서 매번 io_inout 칼럼의 값을 표기해야만 매입인지 매출인지 구별이 된다.
...
-- "거래처별로" 매입과 매출을 구분하여 볼수 있는 Query를 완성했다.
...
-- 00 회사에서 2019년 1년동안 총 매입, 총 매출, 총 이익이 얼마나 발생했는지 알고싶다.
