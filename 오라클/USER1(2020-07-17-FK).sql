...
-- 현재 스키마에 있는 학생정보와 성적정보 테이블을 FK(외래케) 관계로 설정을 한다.
...
REFERENCES tbl_student(st_num); -- 부모는 tbl_student,
                                -- 연동(연결)할 부모 칼럼은 : st_num
                                
-- 생성된 FK 제약조건을 삭제
ALTER TABLE tbl_score
DROP CONSTRAINT fk_st_sc CASCADE;

-- 학생보에 없는 20200 학생의 국어 성적을 추가했다.
-- 20200 학생의 국어 점수가 학적 정보가 이상이 없는 학생의 성적인지 
-- 확신할수 있는 방법이 없다
-- 따라서 이 데이터는 무결성 원칙에 위배되는 데이터 일수 있다.
...
-----------------------------------------
-- 학생정보 st_num 데이터         성적정보 sc_num 데이터
--      값이 추가(있다)    >>     있다(추가할수 있다)
--      값이 없다          >>     절대 없다(추가할수 없다)
--     있어야 한다.        <<      있더라(추가되어 있다)
...
--      1. 먼저 성적정보에서 변경할 학번의 성적 데이터를 모두 삭제하고
--      2. 학생정보의 학번을 변경하고
--      3. 변경된 학번으로 성적데이터를 추가해야 한다.

-- ALTER TABLE 명령을 수행하게 되면 명령이 시작되는 순간 table의 모든 데이터는 접근이 금지된다
--      만약 저장된 데이터가 매우 많다면
--      상당한 시간이 소요 될것이고, 그 동안 table의 데이터를 필요로 하는
--      App은 실행이 중단된다.
...
-- 학생정보, 학과 정보 간의 FK를 설정하려고 한다.
-- 학과정보는 학과명(코드)이 유일한 값으로 정해져 있다. 
-- 학생정보에는 학생마다 학과가 다르거나 같을 수 있어서 수없이 많은 중복데이터가 있다.
-- 같은 이름의 학과는 존재할수 없다.
-- 부모테이블 : 학과정보는 1 인 경우
-- 자식테이블 : 학생정보의 학과는 N 인 경우가 된다.
ALTER TABLE tbl_student
ADD CONSTRAINT FK_D_ST -- 한 스키마 내부에선 이름이 유일해야 한다.
FOREIGN KEY (st_dept)
