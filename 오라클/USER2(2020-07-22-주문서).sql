...
-- tbl_order Table을 만들면서 여기에 추가도리 데이터들 중에 1개의 칼러믕로는 PK를 만들수 없어서
-- 임의의 일련번호 칼럼을 추가하고 이 칼럼을 PK로 선언했다.
-- 이 상황이 데이터를 추가할때 일일히 O_SEQ 칼럼에 저장된 데이터들을 살펴보고
...
-- 이러한 코드가 필요할때 시스템에 이미 준비되어 있는 DUAL 이라는 DUMMY 테이블을 사용해서 코딩을 한다.
...
-- O00001    C00323     P00001 이러한 데이터를 추가한다면 아무런 제약없이 추가되어 버릴것이다.
INSERT INTO tbl_order(o_seq,o_date,o_num, o_cnum, o_pcode) 
        VALUES (seq_order.NEXTVAL,'2020-07-22','O00001', 'C0032','P00001');
        
SELECT o_num, o_cnum,o_pcode FROM tbl_order;

-- 주문번호, 고객변호, 상품코드 이 3개의 칼럼 묶음의 값이 중복되면 INSERT가 되지 않도록 제약조건을 설정해야 한다.
-- UNIQUE : 칼럼에 값이 중복되어서 INSERT가 되는 것을 방지하는 제약조건
-- 주문테이블에 UNIQUE 제약조건을 추가하기
...
--          이미 중복된 값이 있는데 PK, UNIQUE를 설정하려고 할떄 발생을 한다.
-- UNIQUE 제약조건을 추가하기 위해 마지막에 추가된 데이터를 제거하려고 다음과 같은 SQL을 실행한다.
-- 이 SQL을 실행하면, 삭제되어서는 안되는 중요한 주문정보 1거니 같이 삭제 되어 버릴 것이다.
...
-- 후보키중에 단일칼럼으로 PK를 설정할수 없는 상황이 발생을 하면
-- 복수의 칼럼으로 PK를 설정을 하는데
-- UPDATE나 DELETE 를 수행할떄 WHERE 칼럼1 = 값 AND 칼럼2 = 값 AND 칼럼3 = 값 .... 과 같은 조건을 부여해야 한다.
