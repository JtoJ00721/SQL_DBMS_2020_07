...
-- CRUD중에서 가장 먼저 수행해야할 명령이 Create 형 명령
-- DML CRUD Create DDL CREATE 명령과 구분을 해야 한다.
-- DDL의 CREATE : 생성이라는 개념이고 운영체제와 밀접한 관련이 있거나,
--      물리적인 요소가 많다.
--      CREATE TABLESPACE, CREATE 와 같이 저장공간,
--      사용자 정보입력 Schema등을 
--      생성하는 명령 절차

--DML(Data manuplation lan, Data Management Lang)에서 Create
--      물리적 저장공간에 실제 발생된 Data를 추가 저장하기
--      아직 저장되지 않은 논리적 개념의 데이터를 로컬리스트에 보관하는 개념
...
--      (학생정보 : 학번, 이름, 학과, 학년, 전화번호, 이름, 나이)
--      저장할 데이터의 최대 길이(크기)를 결정
--      데이터의 Key라는 것을 지정

-- PRIMARY KEY : 데이터의 Key 중에 가장 중요한 항목으로 
--      데이터를 조회할떄 이 항목으로 조건을 부여하면 유일한 값이 추출되는 항목
--      PK는 절대 중복값이 있어서는 안된다.
--      PK는 절대 NULL 값이 있어서는 안된다.

-- 학생데이터를 저장하기 위해 학생정보 테이블을 생성
-- Table 명명규칙 : 보통 tbl 접두사로 시작한다. snack case 형식으로 이름 지정
CREATE TABLE tbl_student (
    -- 말럼(Field) 명명규칙 : 보통 table이름을 줄여서 접두사 시작
...
INSERT INTO tbl_student -- tbl_student 테이블에 데이터를 추가하겠다.
    (st_num, st_name, st_dept, st_grade, st_tel, st_addr, st_age) -- 칼럼을 나열
VALUES
    ('10001','농농이','모험',1,'010-222-2222','구대륙',18); -- 데이터들
    
-- 데이터를 INSERT할때 혹시 실수로 PK 칼럼에 이미 저장된 데이터를
-- 또 INSERT 하려고 하면 UNIQUE 오류가 발생하면서 데이터를 저장하지 않는다.
-- 이로서 PK칼럼에 값이 중복되는 것을 방지하여 데이터 무결성을 유지한다.
...
INSERT INTO tbl_student (st_num, st_name) VALUES ('10001','농농이');
INSERT INTO tbl_student (st_num, st_name) VALUES ('10002','빙빙이');
INSERT INTO tbl_student (st_num, st_name) VALUES ('10003','쩔쩔이');
INSERT INTO tbl_student (st_num, st_name) VALUES ('10004','선생');
INSERT INTO tbl_student (st_num, st_name) VALUES ('10005','희');
INSERT INTO tbl_student (st_num, st_name) VALUES ('10006','솔라');
...
-- 두개의 칼럼을 나열했는데 VALUES에 3개의 ㄷ이터를 지정하면 오류 발생
INSERT INTO tbl_student (st_num, st_name) VALUES ('10007','호옹이', 33);

-- 학번만 데이터를 지정하고 INSERT 수행
-- st_name칼럼이 NOT NULL constraint 설저이 되어 있기 때문에
...
INSERT INTO tbl_student (st_num, st_name) VALUES ('10007','최중의');
...
-- UPDATE 명령어는 특별한 경우가 아니면 항상 PK를 기준으로 하라
UPDATE tbl_student
SET st_dept = '모험'
WHERE st_num = '10001';

-- 학생이름(st_name) 칼럼의 값이 모든 데이터를 찾아서
-- 그 데이터들의 st_dept 말럼 값을 작가로 변경해라
-- 이 명령의 결과로 만약 쩔쩔이 학생의 데이터가 1개밖에 없어서 
-- 원하는 결과를 얻었더라도, 가급적 사용하지 말아야 하는 명령이다.
-- 어떤 이유로 SELECT 문을 수행했을떄 쩔쩔이 데이터가 1개밖에 보이지 않았더라도
-- 만에 하나 감춰진(보이지 않는) 영역에 쩔쩔이 데이터가 또 존재를 한다면
-- 이 명령을 수행하는 순간 이 table의 에이터는 무결성을 잃게 된다.
UPDATE tbl_student
SET st_dept = '작가'
WHERE st_name = '쩔쩔이';
...
-- 학번이 10002, 10005, 10006 인 학생의 학과를 변경하고 싶을때
-- PK를 기준으로 가능하면 1번에 1개씩 데이터를 변경하는 명령어를 수행하는 것이 좋다
UPDATE tbl_student
SET st_dept = '장사'
WHERE st_num = '10002';
...
-- 학번이 10002, 10003, 10005, 10006 인 학생의 학과를 에엫따로 변경하고 싶다.
-- 여러번의 명령을 수행하지 않고 변경할수 있는 방법
...
-- 쩔쩔이 학새의 데이터가 필요가 없어서 삭제를 하려고 한다.

-- 실무에서는 master Table의 데이터는 함부로 삭제를 하지 않는다.
-- 필요가 없어진 데이터는 특정 칼럼에 값을 세팅하여
-- 칼럼 값을 기준으로 필요한 데이터, 필요없는 데이터를 구분한다.

-- table에서 데이터를 삭제하기 CRUD 마지막 Delete 명령(ANSI 표준)
-- DELETE 명령도 UPDATE명령과 마찬가지로 반드시 WHERE 절을 동반하는 형태로
--      명령을 수행해야 한다.
--      그렇지 않으면 영 좋지않은 기간동안 곶통에 시달리게 될것이다.
DELETE FROM tbl_student
WHERE st_num = '10003';

-- INSERT, UPDATE, DELETE 명령을 수행한 후 명령을 확정짓는 명령
-- COMMIT 이후에는 ROLLBACK으로 해당 명령을 취소할수 없다.
COMMIT;
...
-- 데이터를 테이블 형식으로 보여줄때 모든 칼럼을 다 포함하여 보여달라
-- SELECTION type로 table 보기
SELECT * FROM tbl_student;

-- 데이터를 table 형식으로 보여줄때 나열된 칼럼들만 보여달라
-- projection 지정 : SELECT문을 수행핼때 보고자하는 칼럼들을 나열하는 것
SELECT 
    st_num,
    st_dept,
    st_name,
...
UPDATE tbl_student
SET st_dept = '에엫따'
WHERE st_num IN ('10002','10003','10006');

SELECT st_name, st_name, st_name
FROM tbl_student
WHERE st_dept = '에엫따';

-- st_name + "는" + st_dept + "학과입니다"
SELECT st_name || '는 ' || st_dept || ' 학과입니다'
FROM tbl_student
WHERE st_dept = '에엫따';
...
-- 학과가 컴퓨터공학 이거나(OR) 학과가 법학인 학생들만 보여달라
SELECT *
FROM tbl_student
...
-- IN(포함)연산자를 사용하여 다수의 조건을 부여하여 값을 가져오기
SELECT st_name, st_dept
FROM tbl_student
WHERE st_dept IN ('컴퓨터공학', '법학');

-- AND(  학년이 2학년부터 3학년인 학생들
...
-- 문자열 칼럼의 데이터가 모두 길이가 같고 format이 같은경우
-- 부등호(범위연산)를 포함하여 값을 추출, 조회 할 수 있다.
...
-- WHERE st_num >= '2020-01-01' AND st_num <= '2020-10-30';

-- 시작값과 종료값이 포함된 범위를 조회하는 코드는
-- BETWEEN 연산자를 사용하여 조회하는 코드로 변환하여 사용할수 있다.
SELECT st_num, st_name, st_dept, st_grade
FROM tbl_student
