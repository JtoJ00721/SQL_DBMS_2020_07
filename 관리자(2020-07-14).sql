...
/*
    여러줄 주석
    SQL 명령을 입력할때 명령이 끝났다는 것을 알리기 위해서 ; 붙여준다
...
--      DB Object(객체)의 정보를 보관하고 있는 table
SELECT *
FROM tab;
...
-- DBMS가 보관하고 있는 데이터를 table 형식으로 보여달라는 명령
-- DBMS의 DML(Database manuplation lang)중에서 
--       Read(조회)를 수행하는 명령
--      CRUD중에서 R : Read, Retrive 를 수행하는 명령문
SELECT*FROM 주소록;

-- SQL 명령문을 통해서 DB객체를 만들고, 삭제하고, 데이터를 추가, 변경, 삭제를 수행할텐데
-- sys 사용자로 접속을 하게되면 중요한 정보들을 잘못 삭제, 변경할 우려가 있기 때문에
-- 실습을 위하여 사용자를 생성하여 수행을 할 것이다.

-- 사용자를 추가하는 순서
-- 1. table Space : 데이터를 저장할 물리적 공간을 설정
-- 2. user : 사용자를 생성하고, 물리적 저장 공간과 연결
...
-- Size : 오라클에서는 성능의 효율성을 위해 일단 빈 공간을 설정하게 된다.
-- 크기는 최초에 저장할 데이터의 크기를 계산하여 설계하고 설정한다.
-- 너무 작으면 효율이 떨어지고 너무 크면 불필요한 공간을 낭비한다.
...
-- 질의작성기에서 코드를 작성할때 약속
-- DBMS의 SQL문은 특별한 일부 경우를 제외하고 대소문자 구분을 하지 않는다.
-- DBMS, SQL, 오라클과 관련된 키워드는 모두 대문자로 작성할것
-- 면수, 값, 내용 등은 소문자로 사용하며
-- 특별히 대소문자를 구분해야 하는 경우 별도 공지를 한다.
...
INCLUDING CONTENTS AND DATAFILES -- 연관된 정보와 데이터 파일도 같이 삭제하고
CASCADE CONSTRAINTS; -- 드리고 설정된 권한, 역할 등이 있으면 그들도 같이 삭제하라

-- 위에서 생성한 tableSpace를 관리하며, 데이터를 조작할 사용자를 생성
CREATE USER user1 IDENTIFIED BY 1234 -- 사용자 아이디를 user1으로 설정하고 초기 비번을 1234로 설정하라
...
-- user1이 로그인을 수행하고 최소한으로 데이터들을 관리할수 있도록 권한을 부여 하라
-- RESOURCE : 오라클에서 USER에게 줄수 있는 권한 중 상당히 많은 일을 수행할 수 있는 권한
-- 현재 시스템에 설치된 모든 
--      tableSpace를 대상으로 무제한(TableSpace가 허용하는 범위) 저장이 가능한 권한
-- RESOURCE 권한은 Standart, Enterprice DBMS 에서는 함부로 부여되서는 안된다.
-- CONNECT와 RESOURCE 권한을 부여하게 되면 거의 DBA수준의 권한을 갖게된다.
...
-- DBA 권한(Roll)은 SysDBA보다 한단계 낮은 권한 등급을을 가지며
