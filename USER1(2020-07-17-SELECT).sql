...
-- 현재 학과 테이블의 학과명주에 관광학 학과를 관광정보학으로 변경을 하려고 한다.
...
--      UPDATE 할떄 d_name = '관광학' 조건으로 UPDATE를 실행하면 안된다.
--      UPDATE tbl_dept SET d_name = '관광정보학' WHERE d_name = '관광학' 처럼
--          명령을 수행하면 안된다.
-- 3. 조회된 결과에서 PK값이 무었인지를 파악해야 한다.
...
-- DELETE
-- DBMS의 스키마에 포함된 Table중에 여러 업무를 수행하는데 필요한 테이블을
--      보통 Master Data Table이라고 한다.
--      (학생정보, 학과정보)
--      Master Data는 초기에 INSERT가 수행된 후에 업무가 진행되는 동안
--      가급적 데이터를 변경, 삭제하는 일이 최소화 되어야 하는 데이터이다
...
--      통계, 집계 등 보고서를 작성한 후 데이터를 검증하였을떄 이상이 있으면
...
UPDATE tbl_score
SET sc_kor = 90, sc_math = 90 -- 다수의 칼럼 값을 변경하고자 할떄 칼럼 = 값, 칼럼 = 값 형식으로
WHERE sc_num = '20015';
...
-- UPDATE나 DELETE는 수행한 직후 반드시 SELECT하여
...
-- 아래 Query문을 실행했을떄 
-- 힉셍정보는 보이는데, 성적정보 칼럼 값들이 모두 (null)로 나타나면
...
-- 성적데이터의 국어점수가 가장 놓은 값과 가장 낮은 값은 무었인가??
...
-- 최고점수는 100점, 최저잠수는 51이다
-- 이 점수를 받은 학생은 누구일까
SELECT ST.st_num AS 학번, ST.st_name AS 이름, SC.sc_kor AS 국어점수
FROM tbl_student ST
    LEFT JOIN tbl_score SC
        ON ST.st_num = SC.sc_num
WHERE SC.sc_kor = 100 OR SC.sc_kor = 51
ORDER BY SC.sc_kor DESC, ST.st_num ASC;
...
-- sub query는method, 함수를 호출하는 것과 같이 sub query return 해주는 값을 
-- 칼럼과 비교하여 최종 결과물을 낸다
SELECT SC.sc_num AS 학번, ST.st_name AS 이름, SC.sc_kor AS 국어점수
FROM tbl_score SC
...
-- WHERE 절과 GROUP BY 절에서는 ALias로 설정된 칼럼 이름을 사옹할 수 없다.
-- ORDER BY 에서는 Alias로 설정된 칼럼 이름을 사용할 수 있다.
SELECT sc_num, sc_kor, sc_eng, sc_math, sc_music, sc_art,
        (sc_kor+ sc_eng+ sc_math+ sc_music+ sc_art) AS 학생총점,
...
(
    SELECT AVG((sc_kor+ sc_eng+ sc_math+ sc_music+ sc_art)/5)
    FROM tbl_score
) AND sc_num < '20020' ORDER BY 학생평균 ASC, sc_num ASC;
...
-- 추출된 학급에서 반 명칭이 '2006' 보다 작은값을 갖는 반만 추출
-- HAVING : 성질이 WHERE 과 매우 비슷하다.
...
-- 2001 ~ 2005까지는 A 그룹, 2006 ~ 20010까지는 B 그룹일떄
-- 반명이 '2005' 이하, A 그룹의 반들 평균 구하기
SELECT SUBSTR(sc_num,1,4) AS 반, ROUND(AVG((sc_kor+sc_eng+sc_math)/3)) 반평균
FROM tbl_score
GROUP BY SUBSTR(sc_num,1,4)
...
-- 어쩔수 없이 통계결과를 제한할떄는 HAVING을 써야한다.
