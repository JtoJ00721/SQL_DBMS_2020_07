...
-- 		255까지 가능 
-- VARCHAR : 한글을 포함한 가변형 문자열을 저장하는 컬럼 
-- 		65565까지 가능 

-- 정수형 : INT(4byte, 2의 32승까지), BIGINT(8byte, 무제한), 
...
-- MySQL에서는 데이터와 관련된 BBL, DML, DCL 등의 명령을 수행하기 전데
-- 사용할 Scheme를 Open하는 절차가 필요하다 
...
SELECT io_bcode, 
	...
	SUM(CASE WHEN io_inout = '매출' THEN io_amt ELSE 0 END) AS '먜출합계'
FROM tbl_iolist 
GROUP BY io_bcode;
...
-- 현재 MySQL서버에 만들어진 스키마(Database)리스트
SHOW DATABASES;
...
INSERT INTO tbl_score(sc_num, sc_scode,sc_sname, sc_score)
		VALUES('20001','D003','수학',90);
        
INSERT INTO tbl_score(sc_num, sc_scode,sc_sname, sc_score)
		VALUES('20002','D001','국어',80);
INSERT INTO tbl_score(sc_num, sc_scode,sc_sname, sc_score)
		VALUES('20002','D002','영어',70);
INSERT INTO tbl_score(sc_num, sc_scode,sc_sname, sc_score)
		VALUES('20002','D003','수학',90);


SELECT * FROM tbl_score;

-- 한개의 칼럼으로 PK를 만들떄는 설계할떄 고려를 하고
-- Table을 생성할떄 만드는 것이 편리하다.
...
-- 복합키로 PK를 설정해야 하는데 왠지 그러기 싫다
-- 임의 실제 데이터와는 관계가 없는 별도의 PK만을 위한 칼럼을 만들고
