...
--      만약 어떤 상품명이 변경이 필요한 경우
--      상품명을 UPDATE하여야 하는데, 2개 이상의 레코드를 대상으로 UPDATE 과정이 필요하다.
--      2개 이상의 레코드를 UPDATE 수행하는 것은 데이터의 무결성을 해칠수 있는 수행이 된다.
--      이러한 문제를 방지하기 위해 다음과 같은 정규화를 수행한다.

-- 상품명 정보를 별도의 테이블로 분리하고, 상품정보에 상품코드를 부여한 후
--      tbl_iolist와 연동하는 방식으로 저규화를 실행한다.
...
-- 위 Query를  실행하여 P.p_name 항목에 NULL값이 있다면 상품정보 테이블이 잘못만들어진 것이다.
-- 상품정보 테이블을 삭제하고 과정을 다시 수행해야 한다.

-- 쿼리 결과중에 P.p_name 항목의 값이 NULL인 리스트만 보여라
-- 쿼리결과가 정상이면 리스트는 한개도 없어야 한다.
SELECT io.io_pname, P.p_name
FROM tbl_iolist IO
    LEFT JOIN tbl_product P
        ON IO.io_pname = P.p_name
WHERE P.p_name IS NULL;     -- 반대(해당칼럼 값이 NULL이 아니냐) : IS NOT NULL

-- 이제 상품데이터가 이상 없음을 알았으니 매입매출 테이블에 상품코드를 저장할 "칼럼을 추가"
-- ALTER TABLE : 테이블의 구조(칼럼 추가, 삭제)변경, 칼럼의 타입 변경 등을 수행하는 명령
...
-- NOT NULL 조건을 추가하기
--1. p-code 칼럼을 추가, 기본값으로 문자열이므로 빈칸을 추가(숫자 칼럼이면 0을 추가)
-- p_code 칼럼을 추가, 기본값으로 빈칸을 추가(숫자 칼럼이면 0을 추가)
ALTER TABLE tbl_iolist ADD (io_pcode CHAR(5) DEFAULT ' ');

-- 2. p_code 칼럼의 제약조건을 NOT NULL 로 설정
...
--          데이터가 잘리거나, 문자열이 알수없는 값으로 변형되는 경우가 발생할 수 있다.

-- 칼럼의 이름변경 하기
-- io_pcode의 이름을 io_pcode1으로
ALTER TABLE tbl_iolist RENAME COLUMN io_pcode TO io_pcod1;
ALTER TABLE tbl_iolist RENAME COLUMN io_deco TO io_dceo;


-- 상품정보에서 매입매출장 각 레코드의 상품명과  일치하는 상품코드를 탖아서
-- 매입매출장의 상품코드(io_pcode) 칼럼에 저장해주기



-- UPDATE 명령이 SubQuery를 만나면
-- 1. subQuery에서 현재 iolist의 io_pname 칼럼의 값을 요구하고 있다.
...
--      이때 Sub Query는 반드시 1개의 칼럼, 1개의 VO만 추출되어야 한다.
...
-- iolist에 저장된 dname 칼럼으로 거래처정보에서 데이터를 조회하고
-- iolist에 거래처 코드칼럼으로 UPDATE
