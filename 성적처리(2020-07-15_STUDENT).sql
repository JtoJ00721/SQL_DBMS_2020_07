...
-- INSERT 를 하지 않는다 ==> 데이터의 무결성을 유지한다.
...
-- CHECK : 어떤값이 해당범위에서 유효한 경우만 추가 가능

-- 문자열 칼럼의 Type
-- CHAR(자릿수) : 저장하는 데이터의 길이가 모두 일정할 경우 사용하는 Type
--      저장하는 데이터가 자릿수가 같지 않으면 부족한 부분을 공백으로 채우는데
--      DB에 따라 문자열의 앞, 뒤에 공백을 추가하여 간혹 조회를 할떄
--      조회가 잘 안될수도 있다.
--VARCHAR2(자릿수) : 저장하는 데이터의 길이가 일정하지 않을경우 사용하는 type이다
...
-- unique constraint (GRADE.SYS_C007009) violated : PK나 UNIQUE로 설정된 칼럼에
--      이미 저장된 값을 또 저장하려고 할떄
...
-- 예) 나열된 칼럼의 개수는 6개인데, values 데이터는 6개 미만으로 설정된 경우
...
--      만약 NUMBER로 설정된 칼럼에 숫자를 '' 따옴표로 묶어서 지정하면
--      내부에서 문자열 > 숫자 형식으로 저장한다 : 자동형변환
--      그런데 문자열 > 숫자 형식으로 자동형변환 하는데 숫자로 변환할수 없는 
...
--      CHECK로 유효성 검사를 설정했는데 범위, VAlid를 통과하지 못했을떄
